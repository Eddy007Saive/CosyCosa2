...
import { Plus, Edit, Trash2, Eye, EyeOff, Save, X, Loader2, Upload, ImageIcon } from 'lucide-react';
    ...
    <div className="space-y-6" data-testid="blog-tab">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gérez vos articles SEO</p>
        </div>
        <Button onClick={() => openModal()} className="bg-[#2e2e2e] text-white hover:bg-black" data-testid="add-blog-btn">
                ...
                <TableRow key={post.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => deletePost(post.id)} className="text-red-500 hover:text-red-700">
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 mt-4">
            <div>
              ...
              <Input value={form.title} onChange={(e) => handleTitleChange(e.target.value)} className="mt-1" placeholder="Conciergerie CosyCasa à..." data-testid="blog-form-title" />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Input value={form.slug} onChange={(e) => setForm(prev => ({ ...prev, slug: e.target.value }))} placeholder="conciergerie-cosycasa-a-porto-vecchio" data-testid="blog-form-slug" />
              </div>
            </div>

            <div>
              <Label className="text-xs uppercase tracking-widest text-gray-500">Extrait (affiché sur la page listing)</Label>
              <Textarea value={form.excerpt} onChange={(e) => setForm(prev => ({ ...prev, excerpt: e.target.value }))} className="mt-1" rows={3} placeholder="Courte description de l'article..." data-testid="blog-form-excerpt" />
            </div>
              ...
              {form.hero_image ? (
                <div className="mt-2 relative group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    <label className="cursor-pointer bg-white text-[#2e2e2e] px-4 py-2 text-xs uppercase tracking-widest hover:bg-gray-100">
                      <Upload className="w-3.5 h-3.5 inline mr-2" />Remplacer
                      <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                    </label>
                    <button type="button" onClick={() => setForm(prev => ({ ...prev, hero_image: '' }))} className="bg-red-500 text-white px-4 py-2 text-xs uppercase tracking-widest hover:bg-red-600">
                      <X className="w-3.5 h-3.5 inline mr-1" />Supprimer
                    </button>
                  </div>
                </div>
              ) : (
                <label className="mt-2 flex flex-col items-center justify-center h-40 border-2 border-dashed border-gray-300 hover:border-gray-400 cursor-pointer bg-gray-50 hover:bg-gray-100" data-testid="blog-form-hero-image">
                  {uploading ? <Loader2 className="w-8 h-8 animate-spin text-gray-400" /> : (
                    <>
            ...
            <div>
              <Label className="text-xs uppercase tracking-widest text-gray-500 mb-2 block">Contenu</Label>
              <Textarea
                value={form.content}
            ...
            <div className="border-t border-gray-200 pt-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={form.meta_title} onChange={(e) => setForm(prev => ({ ...prev, meta_title: e.target.value }))} className="mt-1" placeholder="Titre pour les moteurs de recherche" data-testid="blog-form-meta-title" />
                ...
                <div>
                  ...
                  <Textarea value={form.meta_description} onChange={(e) => setForm(prev => ({ ...prev, meta_description: e.target.value }))} className="mt-1" rows={2} placeholder="Description pour les moteurs de recherche" data-testid="blog-form-meta-desc" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={form.author} onChange={(e) => setForm(prev => ({ ...prev, author: e.target.value }))} className="mt-1" data-testid="blog-form-author" />
              </div>
              <div className="flex items-end gap-3">
                <div className="flex items-center gap-2">
                  <Switch checked={form.is_published} onCheckedChange={(v) => setForm(prev => ({ ...prev, is_published: v }))} data-testid="blog-form-published" />
